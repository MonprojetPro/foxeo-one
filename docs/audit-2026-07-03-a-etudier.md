...
| 5 | **Backups planifiés** | `backup-weekly` est maintenant déployée + bucket `backups` créé ; le bouton manuel du Hub fonctionne | Un cron pg_cron hebdomadaire pour l'automatiser + une politique de rétention | ⭐⭐⭐ C'est ta prod — filet de sécurité indispensable avant les vrais clients |
