...
| 5 | **Backups planifiés** | ~~UI + moteur~~ ✅ FAIT le 2026-07-03 : `backup-weekly` déployée, bucket `backups` créé, onglet Backups branché (composant `BackupStatus`, backup manuel fonctionnel) | Reste : un cron pg_cron hebdomadaire pour l'automatiser + une politique de rétention | ⭐⭐⭐ C'est ta prod — filet de sécurité indispensable avant les vrais clients |
