...
## UX-00X — Un écran = un seul porteur de message

- **Date** : 2026-07-31
- **Categorie** : UX / Conception d'interface
- **Symptome** : sur un client résilié, la page « Mon Parcours » affichait **trois encarts qui disaient la même chose** en moins d'un écran de haut : le bandeau ambre global du layout (« Votre abonnement est terminé »), le bandeau d'Élio le Concierge (« Ton abonnement est terminé, alors ton parcours s'arrête ici… »), puis un troisième encart ambre sous la barre de progression (« Ton parcours est arrêté. »). MiKL : *« on a dit qu'on arrêtait les 1000 encarts, l'encart d'Élio est là pour ça, c'est lui le porteur de message. »*
- **Cause racine** : chaque encart a été ajouté **isolément et légitimement**, à trois moments différents, chacun par un développeur (ou une session) qui regardait son composant sans regarder l'écran assemblé. Le commentaire laissé dans `parcours-overview.tsx` le dit noir sur blanc : *« Le bandeau ambre global du layout dit l'abonnement ; celui-ci dit le parcours »* — une distinction réelle en conception, **invisible pour l'utilisateur** qui lit trois fois la même chose. La redondance n'est jamais visible depuis le composant : elle n'existe qu'à l'écran.
- **Solution validee** : désigner **un porteur unique par écran** — ici Élio, qui a déjà une voix et une place identifiée. Les autres se taisent *sur cet écran-là*. Mais surtout : **ne pas supprimer le message, le déplacer**. Le bandeau global reste sur tous les écrans sans Élio (documents, factures, support, réglages) via un slot qui filtre sur le `pathname` — sans lui, un client résilié sur `/modules/documents` n'aurait plus **aucune** indication de son état. Et Élio One, qui ne disait rien du tout de la résiliation, a dû apprendre à le dire avant qu'on puisse lui retirer le bandeau.
- **Regle a suivre** : (a) avant d'ajouter un encart d'état, **regarder l'écran assemblé, pas le composant** — s'il existe déjà une voix (Élio, un bandeau, un badge), l'utiliser au lieu d'en empiler une nouvelle. (b) Retirer un encart redondant = **vérifier d'abord que le porteur restant couvre TOUS les écrans concernés** ; sinon on retombe sur la leçon « un verrou sans l'interface est pire que rien ». (c) Un filtre par route mérite un test unitaire : une route de trop dans la liste et l'information disparaît silencieusement d'un écran entier.
- **Agents impliques** : MAX, PIXEL, ATLAS

---

## IA-00X — Un prompt ne contient que ce qu'on y met : ce qui n'est pas dit est deviné

- **Date** : 2026-08-01
- **Categorie** : IA / Prompts
- **Symptome** : Élio écrivait au client **« MiKL a bien épluché ta cible et ELLE te propose quelques ajustements »**. Repéré par hasard en régénérant des contenus pour les captures du site. Reproduit à froid : **2 générations sur 10** mettaient MiKL au féminin (Haiku, T=0.7).
- **Cause racine** : **aucun** des prompts d'Élio ne déclarait le genre de MiKL. Le modèle devait le déduire du seul prénom « MiKL » — graphie inhabituelle, aucun signal fiable — et tranchait au hasard d'une génération à l'autre. Le défaut est particulièrement sournois : il ne lève aucune erreur, ne casse aucun test, n'apparaît qu'une fois sur cinq… et s'adresse directement au client, qui connaît MiKL personnellement. Un contenu généré non déterministe **n'échoue pas, il dérape** — donc rien ne le signale.
- **Solution validee** : constante partagée `OPERATOR_IDENTITY_RULE` dans `@monprojetpro/utils` (et non dans un module : `module-parcours` et `module-elio` en ont tous deux besoin, or un module ne peut pas en importer un autre). Injectée aux **7 points d'entrée LLM** recensés : `BASE_PROMPT` (couvre à lui seul les 4 chats Lab/One/Hub/agent Hub), les deux générateurs de mots proactifs, `COACH_GUARDRAILS` (couvre les 13 agents d'étape **sans toucher la base**, puisqu'il préfixe leur prompt), la génération de documents, et `generate-draft`. Vérifié par **A/B réel sur 10 générations** de l'événement exact qui avait échoué : 2/10 avant → 0/10 après.
- **Regle a suivre** : (a) tout attribut qu'un prompt suppose « évident » (genre, rôle, ton, unité, devise, fuseau) doit être **écrit** — le modèle ne connaît pas le contexte hors de son prompt. (b) Corriger le prompt où le bug s'est montré ne suffit jamais : **recenser tous les appels LLM** (`grep "invoke('elio-chat'"`), puis vérifier lesquels parlent du sujet concerné. Ici 7 points d'entrée pour un seul symptôme observé. (c) Un fix de prompt se **prouve par A/B sur N générations**, jamais par relecture : la sortie n'est pas déterministe, et « ça a l'air d'aller » sur un tirage ne dit rien. (d) Attention aux faux positifs du détecteur : la première version de mon test comptait le « elle » de « reprendre ses mots à elle » (qui parlait de la persona du client) et donnait un résultat absurde — le détecteur doit cibler le mot en position de sujet du verbe concerné.
- **Agents impliques** : MAX, ATLAS

## 2026-08-01 — OAuth Google : le mode « Test » tue les jetons au bout de 7 jours

- **Categorie** : Integration tierce / OAuth / Diagnostic
- **Symptome** : agenda du Hub — « Token expiré pour culus.osteo@gmail.com » alors que l'ecran Parametres affichait « Connecte » en vert, et qu'aucun bouton de reconnexion n'existait. MiKL : « je n'ai pas de fonction reconnexion ».
- **Cause racine (confirmee par MiKL)** : l'application OAuth etait en statut **« Test »** dans la Google Cloud Console. Dans ce mode, Google **invalide les refresh tokens au bout de 7 jours**. Signature reconnaissable dans les donnees : `created_at` 2026-04-03, dernier refresh reussi 2026-04-09 — soit exactement 6 jours, puis plus rien pendant des mois.
- **Ce qui a permis de trouver** : lire la base AVANT de toucher au code (`refresh_token` present ? `token_expires_at` ? ecart creation/dernier refresh ?). Le `refresh_token` etait bien la — donc ce n'etait ni un `prompt=consent` manquant ni un bug de stockage, contrairement a ce que le message d'erreur laissait croire.
- **Regle a suivre** :
  (a) Un ecart de ~7 jours entre la connexion et le dernier renouvellement reussi d'une integration Google = **mode Test**, avant toute autre hypothese. Verifier le statut de publication OAuth.
  (b) Passer en Production ne **ressuscite pas** un jeton deja revoque, et ne requalifie pas retroactivement un jeton emis pendant le mode Test : il faut **reconnecter chaque compte** apres la bascule.
