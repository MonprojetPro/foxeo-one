...
- **Solution** : **broadcast depuis la base** (même pattern éprouvé que `broadcast_documents_change`) — trigger `broadcast_parcours_change` → `realtime.send(payload, 'parcours_changed', 'parcours:'||client_id, false)` sur `client_parcours_agents` / `parcours` / `step_submissions`. Le hook client s'abonne au canal `parcours:{clientId}` en `broadcast` et invalide `['parcours', clientId]`. Le broadcast contourne la RLS par-ligne ; la donnée réelle reste re-fetchée via une requête serveur RLS-protégée (aucune fuite : le payload est un simple signal de refetch).
