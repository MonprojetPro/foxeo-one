...
| T-023a | Repondre AVEC des fichiers depuis le Hub (2e sens du perimetre elargi par MiKL le 10-09). Bloque en amont : le guichet MenuFacile n'expose pas encore d'URL d'upload pre-signee ni `attachment_ids` sur `/reply` — c'est l'item F-030c du board MenuFacile, non demarre. Piege deja identifie la-bas : les policies Storage ne laissent lire que le dossier `<auth.uid()>/…`, donc un fichier depose par l'operateur ailleurs serait invisible cote utilisateur | Feature | Filiation T-023 | Should | Interne | M | S/M | Qualifie — bloque par F-030c (projet MenuFacile) | — | 2026-09-15 | — | 2026-09-15 |
...
| Implementer les endpoints « envoi de fichiers PAR L'OPERATEUR » du guichet MenuFacile (URL d'upload pre-signee + `attachment_ids` sur `/reply`) — item F-030c du board MenuFacile. Le volet LECTURE est livre depuis le 10-09 (admin-api v13) et consomme par le Hub depuis le 15-09 ; ne reste que ce sens-la | Projet MenuFacile (MiKL) | T-023a | 2026-09-10 |
