...
| T-023 | Appel reel au guichet avec un fil portant des pieces jointes | NON VERIFIE — la sonde exige `MENUFACILE_ADMIN_API_URL` et la lecture du secret dans `.env.local`, fichier sensible non ouvert sans accord de MiKL. L'URL de base est connue (elle figure en clair dans le brief cote MenuFacile), le blocage est l'accord, plus l'information | non verifie |
...
| Autoriser la SONDE du guichet : l'URL de base n'est plus inconnue (`https://hyrtpsgdxwiejcpzqndy.supabase.co/functions/v1/admin-api`, ecrite en clair dans le brief cote MenuFacile), mais l'appel reel exige de lire `MENUFACILE_ADMIN_API_SECRET` dans `.env.local` — fichier sensible, jamais ouvert sans accord explicite. Ce n'est plus un manque d'information mais un feu vert a donner. Sans lui, l'affichage des pieces jointes reste « livre au code, non prouve » | MiKL | T-023 | 2026-09-11 |
