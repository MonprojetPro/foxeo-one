...
| Deployer le Hub pour que l'affichage des pieces jointes soit visible par MiKL : le code est sur `master` et prouve contre le guichet reel, mais rien ne change dans le cockpit tant que le deploiement n'est pas fait. A confirmer aussi : `MENUFACILE_ADMIN_API_URL` doit exister dans les variables Vercel du Hub (elle est absente du `.env.local`, l'appel local passait par une URL en dur dans la sonde) | MiKL | T-023 | 2026-09-15 |
