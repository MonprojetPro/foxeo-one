...
import { useOfficialRecipeActions } from '../hooks/use-official-recipes'
import type {
...
function initialState(recipe: OfficialRecipeListItem | null): FormState {
  return {
    name: recipe?.name ?? '',
    seasons: [],
...
export function RecipeFormModal({
  ...
  const addIngredient = () =>
    set('ingredients', [...f.ingredients, { name: '' }])
  ...
  const cleanSteps = () =>
    f.steps.filter((s) => s.text.trim()).map((s, idx) => ({ ...s, position: idx }))
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4 text-sm">
          ...
          {/* Saisons */}
          <div className="space-y-1.5">
            <Label>Saisons {isEdit ? '(laisser vide = inchangé)' : '*'}</Label>
