...
describe('getEffectiveStepConfig', () => {
  ...
  it('retourne le contexte non-consommé quand il existe', async () => {
    const agentChain = makeChain({
      data: {
        id: 'cpa-1',
        elio_lab_agent_id: AGENT_ID,
    ...
    const contextChain = makeChain({
      data: {
        id: CONTEXT_ID,
        context_message: 'MiKL a préparé un briefing pour cette étape : ...',
      },
      error: null,
    })
    ...
    expect(result.error).toBeNull()
    expect(result.data?.announcementMessage).toBe('MiKL a préparé un briefing pour cette étape : ...')
  ...
  it('retourne la config globale en fallback quand aucun agent n\'est assigné (source: global)', async () => {
    ...
    // 2ème requête : client_step_contexts → pas de contexte
    const contextChain = makeChain({ data: null, error: null })
    // 3ème requête : elio_configs → config globale
    ...
    mockFrom.mockImplementation(() => {
      ...
      if (callCount === 2) return contextChain
      return globalChain
    })
