 ...
 *
 * ⚠️ « Dernier mot » signifie ici « dernier mot NON RÉPONDU » (2026-08-20). Une prise de
 * nouvelles (`project_checkin`) pose une question ; dès que le client y répond, elle est
 * marquée `answered_at` et cesse d'être affichée — le mot précédent (livraison, évolution
 * de l'outil…) reprend alors sa place, au lieu de rester enterré à vie sous un « ça va ? ».
 * Les mots non répondables (livraison, graduation…) ont toujours `answered_at IS NULL` et
 * restent donc affichés indéfiniment, comme avant.
 *
...
export async function getOneConciergeWord(clientId: string): Promise<ConciergeWord | null> {
  ...
  try {
    ...
    const { data: lastWord, error } = await supabase
      ...
      .select('id, body, event_type, agent_label, created_at')
